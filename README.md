...
Com o trace atual, os resultados esperados são:
...
O trace mistura localidade temporal, itens quentes recorrentes e varreduras de itens frios. Por isso, as policies reagem de formas diferentes.

- `fifo`: é simples e barato, mas remove apenas por ordem de chegada. Por não considerar recência nem frequência, ficou abaixo de LRU e LFU na maior parte dos resultados.
- `lru`: aproveita bem localidade temporal. Isso explica sua vantagem nas capacidades pequenas e médias, como em 50 e 100, onde manter os itens usados recentemente ajuda mais.
- `lfu`: precisa de mais espaço para o histórico de frequência ser útil. Começa pior em caches pequenas, mas supera os outros algoritmos a partir de 250 porque preserva melhor os itens mais acessados.

Em resumo, LRU se destacou quando a cache precisava acompanhar o conjunto de trabalho recente; LFU foi melhor quando havia capacidade suficiente para manter itens frequentes; FIFO serviu como referência simples, mas menos adaptativa.
